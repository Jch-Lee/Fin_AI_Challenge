...
def main():
    """메인 함수"""
    logger.info("="*60)
    logger.info("Starting Qwen2.5-14B Relaxed Stopword Question Generation")
    ...
    # 샘플링
    sampled_chunks = sample_diverse_chunks(chunks, n_samples=150)
    ...
    with tqdm(total=30, desc="Generating questions") as pbar:
        for q_type, target_count in question_types.items():
            ...
            while type_counts[q_type] < target_count and retry_count < max_retries:
                ...
                # GPU 메모리 정리
                if device == "cuda" and len(all_questions) % 5 == 0:
