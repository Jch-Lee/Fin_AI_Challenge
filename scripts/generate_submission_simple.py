...
def main():
    ...
    print("제출파일 생성 시작:", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    ...
    for idx, row in test_df.iterrows():
        ...
        try:
            ...
            contexts = [doc.get('content', '') for doc in contexts_list] if contexts_list else []
    ...
    submission_path = f"submission_8bit_simple_{timestamp}.csv"
    
    submission_df.to_csv(submission_path, index=False, encoding='utf-8-sig')
    
    # 백업 저장
    backup_path = f"test_results/submission_8bit_simple_backup_{timestamp}.csv"
