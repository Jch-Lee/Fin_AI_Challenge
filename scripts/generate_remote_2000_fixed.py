...
# -*- coding: utf-8 -*-
"""
Qwen2.5-14B 완화된 불용어 처리 버전 (2000개 질문 생성용)
- 특수문자와 메타텍스트 위주의 필터링
- 파라미터: temperature=0.3, max_new_tokens=256, top_p=0.9
- 간소화된 출력 형식 (id, question만)
- 200개씩 10배치 = 2000개 질문
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('generation_14b_relaxed.log'),
...
def sample_diverse_chunks(chunks: List[Dict], n_samples: int = 150) -> List[Dict]:
    """다양성 기반 청크 샘플링"""
...
def main():
    """메인 함수"""
    logger.info("="*60)
    logger.info("Starting Qwen2.5-14B Relaxed Stopword Question Generation (2000 questions)")
    ...
    # 샘플링 - 2000개 질문을 위해 더 많은 청크 준비
    sampled_chunks = sample_diverse_chunks(chunks, n_samples=500)
    
    # 질문 유형 분포 (2000개 목표)
    question_types = {
        'definition': 400,    # 6 → 400 (원본 비율 유지)
        'process': 400,      # 6 → 400
        'regulation': 400,   # 6 → 400
        'example': 267,      # 4 → 267
        'comparison': 267,   # 4 → 267
        'application': 266   # 4 → 266
    }  # 총 2000개
    ...
    chunk_idx = 0
    max_retries = 5000  # 2000개 생성을 위해 충분한 재시도
    retry_count = 0
    
    logger.info("Generating questions with relaxed stopword filtering...")
    logger.info(f"Critical stopwords count: {len(CRITICAL_STOPWORDS)}")
    
    with tqdm(total=2000, desc="Generating questions") as pbar:
    ...
    output_file = output_dir / f"relaxed_14b_questions_{timestamp}.csv"
